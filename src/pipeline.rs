...
use crate::{VertexColor, UniformStorage, load_glsl};
...
impl SitePipeline {
    pub fn new(sites: Vec<VertexColor>, display: &Display) -> Self {
        ...
        let program =
            glium::Program::from_source(display, &vertex_src, &frag_src, None)
                .unwrap();
        ...
        Self {
           vertexes_buffer, indexes_buffer, program
        }
    }

    pub fn update_vertexes(&mut self, display: &Display, data: Vec<VertexColor>) {
        self.vertexes_buffer = VertexBuffer::new(display, &data).expect(&format!("failed to to build vertex buffer for sites"));
    }
...
impl WirePipeline {
    ...
    pub fn update_vertexes(&mut self, display: &Display, data: Vec<VertexColor>) {
        self.vertexes_buffer = VertexBuffer::new(display, &data).expect(&format!("failed to to build vertex buffer for wires"));
    }
...
impl RegionPipeline {
    pub fn new(regions: Vec<VertexColor>, display: &Display) -> Self {
        ...
        let program =
            glium::Program::from_source(display, &vertex_src, &frag_src, None)
                .unwrap();
        let vertexes_buffer = VertexBuffer::new(display, &regions).expect(&format!("failed to to build vertex buffer for regions"));
    ...
    pub fn update_vertexes(&mut self, display: &Display, data: Vec<VertexColor>) {
        self.vertexes_buffer = VertexBuffer::new(display, &data).expect(&format!("failed to to build vertex buffer for regions"));
    }
